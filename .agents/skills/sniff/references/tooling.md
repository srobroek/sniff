...
The detection engine. The agent (or `independent scout`) selects tools from here, runs
...
1. **cwd + paths.** Tools run with cwd = the **target repo root** (or the
   ...
   (semgrep rules, configs) live at `./references/semgrep-rules/` and
...
- **Invocation:** the shipped ruleset is under `./references/semgrep-rules/`,
