...
export const sniffToolInputSchemas = {
  ...
  sniff_install_tools: { type: "object", properties: { mode: { enum: ["probe", "diagnose", "list", "install"], description: "probe (default), inventory-only diagnose, list, or install" }, bundles: { type: "array", items: stringSchema, uniqueItems: true, maxItems: MAX_INPUT_ARRAY_LENGTH, description: "Required/install bundle names: core dup security rust go python js-ts shell sql css data api infra docs" }, all: { type: "boolean", description: "Select every bundle" }, dryRun: { type: "boolean", description: "Print install commands without running them" }, path: { ...stringSchema, description: "Repo cwd for project-local tools" } }, additionalProperties: false },
  ...
  sniff_read_report_artifact: { type: "object", properties: { readCapability: { ...stringSchema, description: "Opaque report artifact read capability returned by sniff_report" }, reportId: { ...stringSchema, description: "Report ID returned by sniff_report" }, relativePath: { ...stringSchema, description: "Artifact relative path from a sniff_report descriptor" }, offset: { type: "integer", minimum: 0, description: "UTF-8 byte offset returned as nextOffset; defaults to zero" } }, required: ["readCapability", "reportId", "relativePath"], additionalProperties: false },
  sniff_read_analyzer_artifact: { type: "object", properties: { analyzerResultId: { ...stringSchema, description: "Analyzer result ID returned by sniff_run_analyzer" }, readCapability: { ...stringSchema, description: "Opaque analyzer artifact read capability returned by sniff_run_analyzer" }, relativePath: { ...stringSchema, description: "Artifact relative path from the analyzer result index" }, sourcePath: { ...stringSchema, description: "Normalized source path for direct lookup" }, offset: { type: "integer", minimum: 0, description: "UTF-8 byte offset returned as nextOffset; defaults to zero" }, maxBytes: { type: "integer", minimum: 4, maximum: MAX_OUTPUT_TEXT_BYTES, description: "Maximum UTF-8 bytes to return" } }, required: ["analyzerResultId", "readCapability"], additionalProperties: false },
} as const;
