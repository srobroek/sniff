...
export function registerReportArtifacts(artifacts: ReportArtifacts): string {
  ...
  reports.set(capability, { reportId: artifacts.report.reportId, contents, descriptors });
  while (reports.size > MAX_REPORT_ARTIFACT_REGISTRY_ENTRIES) {
...
export function readReportArtifact(options: ReportArtifactReadOptions): ReportArtifactReadResult {
