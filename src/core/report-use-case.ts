...
function safeDirectory(directory: string): string {
	...
	let current = root;
	for (const component of requested.slice(root.length).split(sep).filter(Boolean)) {
		current = join(current, component);
		if (existsSync(current) && lstatSync(current).isSymbolicLink()) throw new Error("sniff report output directory cannot traverse a symlink");
...
export async function runSniffReportTool(options: SniffReportToolOptions): Promise<SniffReportToolResult> {
	try {
		...
		if (!options.runtime) throw new Error("sniff_report mode=save requires a trusted save authorization boundary");
		const directory = safeDirectory(options.path ?? "");
		const files = [`${report.reportId}.json`, `${report.reportId}.md`, `${report.reportId}.receipt.json`];
		const unsigned = { directory, manifestId: manifest.manifestId, reportId: report.reportId, files, manifest };
