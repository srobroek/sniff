...
export const ANALYZER_ARTIFACT_TTL_MS = 30 * 60 * 1000;
...
function purgeExpired(now = Date.now()): void {
  for (const [capability, entry] of analyzers) if (entry.expiresAt <= now) analyzers.delete(capability);
}
...
export function registerAnalyzerArtifacts(artifacts: AnalyzerArtifacts, now = Date.now()): string {
  ...
  analyzers.set(capability, { analyzerResultId: artifacts.analyzerResultId, contents, descriptors, sourcePaths, expiresAt: now + ANALYZER_ARTIFACT_TTL_MS });
  while (analyzers.size > MAX_ANALYZER_ARTIFACT_REGISTRY_ENTRIES) {
...
export function readAnalyzerArtifact(options: AnalyzerArtifactReadOptions, now = Date.now()): AnalyzerArtifactReadResult {
  ...
  entry.expiresAt = now + ANALYZER_ARTIFACT_TTL_MS;
