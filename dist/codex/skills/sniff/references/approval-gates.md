...
Tools run with cwd set to the target repository, while shipped assets live under `./`, including `references/semgrep-rules/`. Read them through `./<path>`. When a tool needs a filesystem path, resolve the installed skill directory once and pass an absolute path.
