...
**Execution routing:** clippy, rustc lints, cargo-udeps, and cargo-geiger are operator-direct because they have no `sniff_install_tools` registry entries; invoke their documented commands directly. Keep registry-backed `cargo-machete` on `sniff_run_analyzer`.
